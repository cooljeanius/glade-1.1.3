...
gb_widget_load (GtkWidget * widget, GbWidgetSetArgData * data, GtkWidget * parent)
{
  ...
  /* SPECIAL CODE: Special toolbar buttons are saved with special class names.
     We convert them back to the old Glade style of using the child_name. */
  if (class_id)
    {
      if (!strcmp (class_id, "button"))
	{
	  class_id = "GtkButton";
	  child_name = GladeChildToolbarButton;
	}
      else if (!strcmp (class_id, "toggle"))
	{
	  class_id = "GtkToggleButton";
	  child_name = GladeChildToolbarButton;
	}
      else if (!strcmp (class_id, "radio"))
	{
	  class_id = "GtkRadioButton";
	  child_name = GladeChildToolbarButton;
	}
    }
  ...
  if (data->xml_buffer && data->replacing_widget && child_name)
	  ...
	  )
	{
      ...
      else if (!strcmp (child_name, GladeChildToolbarButton))
	{
		...
		GbWidgetGetArgData * data)
{
  ...
  /* SPECIAL CODE: Special toolbar buttons use special class names in the XML
     file. */
  if (child_name && !strcmp (child_name, GladeChildToolbarButton))
    {
      child_name = NULL;
      if (G_OBJECT_TYPE (widget) == GTK_TYPE_BUTTON)
	class_to_save = "button";
      else if (G_OBJECT_TYPE (widget) == GTK_TYPE_TOGGLE_BUTTON)
	class_to_save = "toggle";
      else if (G_OBJECT_TYPE (widget) == GTK_TYPE_RADIO_BUTTON)
	class_to_save = "radio";
      else
	g_warning ("Invalid special toolbar button class");
    }
			 ...
			 GtkWidget * new_child)
{
  ...
  if (child_name && (!strcmp (child_name, GladeChildToolbarButton)
		     || !strcmp (child_name, "vscrollbar")
		     || !strcmp (child_name, "hscrollbar")))
    {
      child_name = NULL;
    }
	  
  /* If the new widget's child name is GladeChildToolbarButton leave it alone,
     otherwise copy the old child name across. */
  if (!new_child_name || strcmp (new_child_name, GladeChildToolbarButton))
    {
...
#if 0
      g_print ("Replacing child of toolbar\n");
#endif
