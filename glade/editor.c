...
#if 0
  /* FIXME: Test: Add emission hooks. */
  signal_id = gtk_signal_lookup ("button_press_event", GTK_TYPE_WIDGET);
  g_signal_add_emission_hook (signal_id, 0, event_watcher, NULL, NULL);
...
{
#ifdef USE_GNOME
  if (BONOBO_IS_DOCK_ITEM (widget) && BONOBO_DOCK_ITEM (widget)->is_floating)
    {
      *paint_widget = widget;
...
{
  /* Check that the widget is still drawable. Anything could have happened
     before our idle handler got called. */
  if (GTK_WIDGET_DRAWABLE (widget))
...
/* This is a bit hackish. To paint the selection rectangle around a widget
   we we step up the hierarchy and paint it in each X window.
   But if an ancestor is a viewport, we don't go up any further as the
   coordinate systems will be totally different.
   We use GDK_INCLUDE_INFERIORS to draw over child X windows.
   We don't even bother trying to draw the selection in the widgets own X
   window, as lots of widgets are composed of several X windows.
   FIXME: Maybe we could just paint it on the toplevel X window, using
   suitable transformations of the coordinates (and taking viewports into
   account). */
static void
editor_paint_selections (GtkWidget *widget)
{
  ...
  MSG1("In editor_paint_selections: %s", gtk_widget_get_name (widget));
  ...
  while (ancestor)
      ...
      if (g_list_find (selected_widgets, ancestor))
	{
	  ...
	  if (ancestor->parent)
	    {
	      ...
	      /* If we are drawing a widget in a GtkLayout we need to adjust
		 the coordinates based on the scoll offset. */
	      if (GTK_IS_LAYOUT (ancestor->parent))
		{
		  x -= (int) GTK_LAYOUT (ancestor->parent)->hadjustment->value;
		  y -= (int) GTK_LAYOUT (ancestor->parent)->vadjustment->value;
		}
	    }
	  else
	    {
	  ...
	  paint_selection (window, x, y, w, h);
	}
...
static void
paint_selection (GdkWindow * window, gint x, gint y, gint width, gint height)
{
  static GdkGC *gc = NULL;

  /* FIXME: Use a cache of gc's for mult-depth displays. Can use same gc
     for drawables with same root window and depth (i.e. same colormap). */
  if (!gc) {
    GtkWidget *widget;
    ...
    if (gdk_color_black (colormap, &color))
      gdk_gc_set_foreground (gc, &color);
    else
      g_warning ("Couldn't allocate color to draw selections");
  }

  MSG4 ("In paint_selection %i,%i %ix%i", x, y, width, height);
