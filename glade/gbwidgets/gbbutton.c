...
/* Returns the type of the button - NORMAL, TOOLBAR or DIALOG. Note that
   TOOLBAR means it is a special toolbar button. Toolbars can also contain
   NORMAL buttons. */
GladeButtonType
				 ...
				 gchar *icon_p)
{
  ...
  if (data->action == GB_SHOWING)
    {
      if (!label_sensitive)
	gb_widget_output_text (data, label_p, "");
...
gb_button_normal_set_properties	(GtkWidget * widget,
				 ...
				 gchar *icon_p)
{
  gchar *stock_id = NULL, *label, *icon_name;
			       ...
			       GbWidgetWriteSourceData * data)
{
  ...
  if (stock_id)
	...
	{
	  gchar *label_text = glade_util_get_label_text (label_widget);
...
/*
 * SPECIAL TOOLBAR BUTTONS. GtkButton/GtkToggleButton/GtkRadioButton.
 */
...
gboolean
gb_button_toolbar_find_child_widgets (GtkWidget *widget,
				      GtkWidget **label,
				      GtkWidget **icon)
{
  GtkWidget *child;
  ...
  *label = *icon = NULL;
  ...
  child = GTK_BIN (widget)->child;
  g_return_val_if_fail (child != NULL, FALSE);
  g_return_val_if_fail (GTK_IS_BOX (child), FALSE);

  box_children = GTK_BOX (child)->children;
  g_return_val_if_fail (g_list_length (box_children) == 2, FALSE);

  /* Note that the label is added first in gtktoolbar.c which is why
     it is first in the list. */
  *label = ((GtkBoxChild*)box_children->data)->widget;
  *icon = ((GtkBoxChild*)box_children->next->data)->widget;

  g_return_val_if_fail (GTK_IS_LABEL (*label), FALSE);
  g_return_val_if_fail (!GB_IS_GB_WIDGET (*label), FALSE);
  g_return_val_if_fail (GTK_IS_IMAGE (*icon), FALSE);
...
static void
gb_button_toolbar_get_properties (GtkWidget * widget,
				  GbWidgetGetArgData * data,
				  gchar *stock_id_p,
				  gchar *label_p,
				  gchar *icon_p)
{
  ...
  /* In the XML the stock item name is actually saved as the label property,
     and the use_stock property specifies that it is a stock item.
     Also, use_underline needs to be set for non-stock items.
     Note that currently GTK+ doesn't really have property support for special
     toolbar buttons, so it is all hacked, here and in libglade. */
  if (data->action == GB_SAVING)
    {
      if (stock_id)
	{
  ...
  if (stock_id)
  ...
  else
    {
      ...
      /* A special toolbar button must have an icon and a label. */
      if (!gb_button_toolbar_find_child_widgets (widget, &label, &icon))
	{
	  g_warning ("Couldn't find special toolbar button children");
	  return;
	}
	...
	icon_name = gtk_object_get_data (GTK_OBJECT (icon), GladeIconKey);
	/* When saving, we save stock icons as "stock_pixmap". */
	if (data->action == GB_SAVING
	    && glade_util_check_is_stock_id (icon_name))
	  gb_widget_output_icon (data, "stock_pixmap", icon_name);
	else
	  gb_widget_output_icon (data, icon_p, icon_name);
    }
...
static void
gb_button_toolbar_set_icon (GtkWidget *widget,
			    GbWidgetSetArgData * data,
			    GtkWidget *icon_widget,
			    gchar *icon_name)
{
  ...
  if (glade_util_check_is_stock_id (icon_name))
  ...
  else
    {
      ...
      if (pixmap_gbwidget)
	gtk_image_set_from_pixmap (GTK_IMAGE (icon_widget),
				   pixmap_gbwidget->gdkpixmap,
...
static void
gb_button_toolbar_set_stock_id (GtkWidget *widget,
				GbWidgetSetArgData * data,
				gchar *stock_id,
				gchar *label_p,
				gchar *icon_p,
  ...
  if (is_stock_item)
    {
      ...
      gtk_image_set_from_stock (GTK_IMAGE (icon_widget), stock_id, icon_size);
      gtk_label_set_text_with_mnemonic (GTK_LABEL (label_widget), item.label);
      gtk_object_set_data_full (GTK_OBJECT (widget), GladeButtonStockIDKey,
				g_strdup (stock_id), g_free);
    }
  else
    {
      /* Change the button back to a normal toolbar button with a label and
	 an icon. */
      GbWidget *pixmap_gbwidget;
...
void
gb_button_toolbar_set_properties	(GtkWidget * widget,
					 GbWidgetSetArgData * data,
					 gchar *stock_id_p,
					 gchar *label_p,
					 gchar *icon_p)
{
  gchar *stock_id = NULL, *label, *icon_name;
  gboolean apply_stock_id = FALSE, apply_label;
  gboolean free_label = FALSE;
  GtkWidget *label_widget, *icon_widget;

  if (!gb_button_toolbar_find_child_widgets (widget, &label_widget,
					     &icon_widget))
    {
      g_warning ("Couldn't find special toolbar button children");
      return;
  ...
  if (apply_stock_id)
    {
      gb_button_toolbar_set_stock_id (widget, data, stock_id, label_p, icon_p,
				      label_widget, icon_widget);
      goto out;
    }

  icon_name = gb_widget_input_icon (data, icon_p);
  /* When loading we also check for "stock_pixmap". */
  if (!data->apply && data->action == GB_LOADING)
      icon_name = gb_widget_input_icon (data, "stock_pixmap");
  if (data->apply)
    gb_button_toolbar_set_icon (widget, data, icon_widget, icon_name);
...
static void
gb_button_toolbar_write_source (GtkWidget * widget,
				GbWidgetWriteSourceData * data)
{
  GladeWidgetData *wdata = data->widget_data;
  GtkWidget *label_widget, *icon_widget;
  gchar *parent_name, *label_text, *type, *radio_widget_name = NULL;
  gchar *stock_id, *icon_name = NULL;
  GType widget_type;
  gboolean has_icon = FALSE;

  g_return_if_fail (widget->parent != NULL);
  parent_name = (gchar*) gtk_widget_get_name (widget->parent);
  ...
  if (widget_type == GTK_TYPE_BUTTON)
  ...
  else
    {
      ...
      if (find_data.found_widget && find_data.found_widget != widget)
	radio_widget_name = source_create_valid_identifier (gtk_widget_get_name (find_data.found_widget));
    }

  /* If it should start a new group, add a space. */
  if (gb_toolbar_get_new_toolbar_group (widget->parent, widget))
    source_add (data, "  gtk_toolbar_append_space (GTK_TOOLBAR (%s));\n\n",
		parent_name);

  gb_button_toolbar_find_child_widgets (widget, &label_widget, &icon_widget);
  ...
  if (stock_id)
	...
	{
	  source_add (data,
		      "  %s = gtk_toolbar_insert_stock (GTK_TOOLBAR (%s),\n"
		      "                                %s,\n"
		      "                                %s,\n"
		      "                                NULL, NULL, NULL, -1);\n",
		      data->wname,
	...
	{
	  /* GtkToolbar doesn't have builtin support for stock toggle & radio
	     items, so we create the label by looking up the stock id, and
	     we create the icon just like a stock icon below. */
	  source_ensure_decl (data, "  GtkStockItem tmp_stock_item;\n");

	  /* We set the stock item label to "" just in case it isn't found. */
	  source_add (data,
		      "  tmp_stock_item.label = "";\n"
		      "  gtk_stock_lookup (%s, &tmp_stock_item);\n",
		      source_make_string (stock_id, FALSE));

	  label_text = g_strdup ("tmp_stock_item.label");

	  /* We use the stock_id for the icon below. */
	  icon_name = stock_id;
	}
    }
  else
    {
      gchar *label = glade_util_get_label_text (label_widget);
      label_text = g_strdup (source_make_string (label, data->use_gettext));
      g_free (label);

      icon_name = gtk_object_get_data (GTK_OBJECT (icon_widget), GladeIconKey);
    }
  ...
  if (icon_name && *icon_name)
	...
	{
	  source_add (data,
		      "  tmp_toolbar_icon = gtk_image_new_from_stock (\"%s\", gtk_toolbar_get_icon_size (GTK_TOOLBAR (%s)));\n",
		      icon_name, parent_name);
	}
      else
	{
	  source_create_pixmap (data, "tmp_toolbar_icon", icon_name);
	}
    }
  ...
  /* FIXME: GTK+ doesn't set "use_underline" for the label, so we set it here.
     Though I'm not really sure toolbar buttons should have mnemonic accels,
     but the stock items do so we are consistent. */
  source_add (data,
	      "  gtk_label_set_use_underline (GTK_LABEL (((GtkToolbarChild*) (g_list_last (GTK_TOOLBAR (%s)->children)->data))->label), TRUE);\n",
	      parent_name);
  ...
  if (stock_id)
      ...
      if (icon)
	{
	  ...
	  source_add (data,
		      ...
		      "                                          %s, ",
		      data->component_name,
		      label_text ? source_make_string (label_text, data->use_gettext) : "");
  ...
  switch (button_type)
    ...
    case GLADE_BUTTON_NORMAL:
      gb_button_normal_get_properties (widget, data, stock_id_p, label_p,
				       icon_p);
      break;
    case GLADE_BUTTON_TOOLBAR:
      gb_button_toolbar_get_properties (widget, data, stock_id_p, label_p,
					icon_p);
					 ...
					 gchar *relief_p)
{
  ...
  switch (button_type)
    ...
    case GLADE_BUTTON_NORMAL:
      ...
      break;
    case GLADE_BUTTON_TOOLBAR:
      gb_button_toolbar_set_properties (widget, data, stock_id_p, label_p,
					icon_p);
      break;
    case GLADE_BUTTON_DIALOG:
      gb_button_normal_set_properties (widget, data, stock_id_p, label_p,
  ...
  /* Handle the Relief property here, as it is simple. */
  if (button_type != GLADE_BUTTON_TOOLBAR)
    {
...
gb_button_create_popup_menu (GtkWidget * widget, GbWidgetCreateMenuData * data)
{
  ...
  child = GTK_BIN (widget)->child;
  if (child && !GB_IS_PLACEHOLDER (child)
      && !gb_toolbar_is_toolbar_button (widget))
    {
      GtkWidget *menuitem;
				 ...
				 GbWidgetWriteSourceData * data)
{
  ...
  if (data->create_widget)
	...
	{
	case GLADE_BUTTON_NORMAL:
	  gb_button_normal_write_source (widget, data);
	  break;
	case GLADE_BUTTON_TOOLBAR:
	  gb_button_toolbar_write_source (widget, data);
	  set_relief = FALSE;
	  break;
	case GLADE_BUTTON_DIALOG:
	  gb_button_normal_write_source (widget, data);
	  break;
	case GLADE_BUTTON_GNOME_DIALOG:
	  gb_button_gnome_write_source (widget, data);
	  break;
	}
    }
...
{
  gb_button_write_standard_source (widget, data);
}
