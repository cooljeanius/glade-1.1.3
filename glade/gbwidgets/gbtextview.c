...
gb_text_view_create_properties (GtkWidget * widget, GbWidgetCreateArgData * data)
{
	...
	property_add_bool (CursorVisible, _("Cursor Visible:"), _("If the cursor is visible"));
...
gb_text_view_get_properties (GtkWidget *widget, GbWidgetGetArgData * data)
{
	...
	gboolean editable, cursor_visible;
...
gb_text_view_write_source (GtkWidget * widget, GbWidgetWriteSourceData * data)
{
  ...
  if (text && *text)
    source_add (data,
		"  gtk_text_buffer_set_text (gtk_text_view_get_buffer (GTK_TEXT_VIEW (%s)),\n"
		"	%s, -1);\n",
		data->wname, source_make_string (text, data->use_gettext));
  g_free (text);
}
