...
glade_atk_create_property_page	(GtkNotebook *notebook)
{
  ...
  property_add_text (GladeATKDescription, _("Description:"),
		       _("The description of the widget to pass to assistive technologies"), 2);
#if 0
				 ...
				 gint	     indent)
{
  ...
  if (name && *name && g_object_get_data (G_OBJECT (atko), GladeATKName))
    save_atk_property (data, buffer, indent, GladeATKName, name, TRUE);
					 ...
					 GString   * buffer)
{
  ...
  if (name && *name && g_object_get_data (G_OBJECT (atko), GladeATKName))
    {
      glade_atk_source_add (buffer,
			    ...
			    source_make_string (name, data->use_gettext));
    }
  ...
  if (description && *description
      && g_object_get_data (G_OBJECT (atko), GladeATKDescription))
    {
      glade_atk_source_add (buffer,
			    ...
			    source_make_string (description, data->use_gettext));
    }
  ...
  if (GTK_IS_TREE_VIEW (widget))
	  ...
	  && g_object_get_data (G_OBJECT (atko), GladeATKTableCaption))
	{
	  glade_atk_source_add (buffer,
				"  g_object_set (G_OBJECT (atko),\n"
				"                \"AtkObject::accessible-table-caption\", %s,\n"
				"                NULL);\n",
			    source_make_string (table_caption, data->use_gettext));
