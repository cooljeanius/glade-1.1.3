...
create_stock_items (void)
{
  ...
  for (elem = stock_items; elem; elem = elem->next)
	...
	g_print (" %i", sizes[i]);
      ...
      /* Note that we use the stock_id as the keys, without copying it, so
	 they are only valid while stock_items exists. */
...
property_create ()
{
  ...
  gtk_window_move (GTK_WINDOW (win_property), 260, 0);
			 ...
			 gboolean	    *apply)
{
  ...
  /* We want to return the stock_id corresponding to the currently selected
     item. */
  return find_stock_id (GTK_COMBO (widget));
}
			 ...
			 const gchar	    *stock_id)
{
  ...
  listitem = find_stock_item (GTK_COMBO (widget), stock_id);
  if (listitem)
    value = glade_util_gtk_combo_func (GTK_LIST_ITEM (listitem));
				 ...
				 gboolean	    *apply)
{
  ...
  /* We want to return the stock_id corresponding to the currently selected
     item. If it is not a stock item, we return the GtkEntry text. */
  stock_id = find_stock_id (GTK_COMBO (widget));
  if (stock_id)
    return stock_id;
  else
    return gtk_object_get_data (GTK_OBJECT (widget), GbFilenameValueKey);
				 ...
				 const gchar	    *icon)
{
  ...
  listitem = find_stock_item (GTK_COMBO (widget), icon);
  if (listitem)
    {
      value = glade_util_gtk_combo_func (GTK_LIST_ITEM (listitem));
...
static void
add_signals_for_type (GType type, GtkWidget *clist, GdkColor *inactive_fg, GdkColor *inactive_bg, gchar *current_signal)
{
  guint *signals;
  guint nsignals;
  gchar *name;
...
show_signals_dialog (GtkWidget * widget, gpointer value)
{
  ...
  for (i = 0; i < n_interfaces; i++)
    {
      add_signals_for_type (interfaces[i], clist, inactive_fg, inactive_bg, current_signal);
    }
  g_free (interfaces);

  while (type)
    {
      add_signals_for_type (type, clist, inactive_fg, inactive_bg, current_signal);
	  ...
	  if (strlen (handler_text) == 0)
	    {
	      gchar buf[128];
	      gchar *widget_name;
	      widget_name = (char*) gtk_widget_get_name (property_widget);
