				 ...
				 const gchar *tooltip)
{
  ...
  button = gtk_toolbar_insert_stock (toolbar, stock_id, tooltip,
				     NULL, NULL, NULL, -1);
  gb_widget_create_from (button, "GtkButton");
  gb_widget_set_child_name (button, GladeChildToolbarButton);
  wdata = gtk_object_get_data (GTK_OBJECT (button), GB_WIDGET_DATA_KEY);
  wdata->tooltip = g_strdup (tooltip);
  gtk_object_set_data_full (GTK_OBJECT (button), GladeButtonStockIDKey,
