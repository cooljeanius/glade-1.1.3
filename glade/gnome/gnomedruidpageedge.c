					...
					GbWidgetWriteSourceData * data)
{
  ...
  if (title && *title)
    {
      source_add (data,
		  "  gnome_druid_page_edge_set_title (GNOME_DRUID_PAGE_EDGE (%s),\n"
		  "                                   %s);\n",
		  data->wname,
		  source_make_string (title, data->use_gettext));
    }

  if (page->text && strcmp (page->text, ""))
    {
      source_add (data,
		  "  gnome_druid_page_edge_set_text (GNOME_DRUID_PAGE_EDGE (%s),\n"
		  "                                  %s);\n",
		  data->wname,
